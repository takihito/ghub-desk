#!/bin/sh
# ghub-desk installer for Linux and macOS
# Usage: curl -sSL https://takihito.github.io/ghub-desk/install.sh | sh
set -eu

REPO="takihito/ghub-desk"
INSTALL_DIR="${GHUB_DESK_INSTALL_DIR:-${HOME}/.local/bin}"
...
# Resolve version: positional arg > GHUB_DESK_VERSION env var > latest release
...
# Extract and install
tar -xzf "${WORK}/${ARTIFACT}" -C "${WORK}" ghub-desk
if [ ! -d "$INSTALL_DIR" ]; then
  mkdir -p "$INSTALL_DIR" 2>/dev/null || {
    echo "Error: cannot create ${INSTALL_DIR} (permission denied)"
    echo "Choose a writable directory or run with sudo:"
    echo "  curl -sSL https://takihito.github.io/ghub-desk/install.sh | sudo GHUB_DESK_INSTALL_DIR=${INSTALL_DIR} sh"
    exit 1
  }
fi
if [ -w "$INSTALL_DIR" ]; then
  mv "${WORK}/ghub-desk" "${INSTALL_DIR}/ghub-desk"
else
...
echo "Installed to ${INSTALL_DIR}/ghub-desk"
...
echo ""
echo "ghub-desk ${VERSION} installed successfully!"
echo "Run 'ghub-desk version' to verify."
