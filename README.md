...
- Use the global `--debug` flag to enable verbose logging (SQL queries, API requests); add `--log-path <file>` (alias: `--error-log-path`) to append logs to a file and imply `--debug`
