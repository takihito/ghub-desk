...
```bash
# 既定パス (~/.ghub-desk/config.yaml) に設定ファイルのひな形を作成
...
`bin/pull_info.sh` は、組織のすべての情報を取得するための一連の `pull` コマンドを順番に実行するヘルパースクリプトです。
各コマンドの実行前に確認プロンプトが表示されます。`--yes` フラグを付けて実行すると、すべてのプロンプトをスキップして実行できます。
